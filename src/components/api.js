...
export const fetchNearbySchools = async (lat, lng, organisationsform = 'alla') => {
  try {
    const url = `${backendUrl}/api/Forskolan/nearby/${lat}/${lng}?organisationsform=${organisationsform}`;
    const response = await axios.get(url);
